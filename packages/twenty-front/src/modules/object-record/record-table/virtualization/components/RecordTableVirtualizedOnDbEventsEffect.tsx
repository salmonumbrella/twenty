...
import { useListenToDbEventsForQuery } from '@/sse-db-event/hooks/useListenToDbEventsForQuery';
...
export const RecordTableVirtualizedOnDbEventsEffect = () => {
  ...
  useListenToDbEventsForQuery({
    ...
    onDbEvents: (dbEvents) => {
      ...
      const updateEvents = dbEvents.filter(
        (dbEvent) => dbEvent.action === DatabaseEventAction.UPDATED,
      );

      const updatedRecordsWithUpdatedFieldsOnly: ObjectRecord[] = [];

      for (const updateEvent of updateEvents) {
        const updatedRecord = updateEvent.record;
        ...
        const updatedFields = updateEvent.updatedFields ?? [];
